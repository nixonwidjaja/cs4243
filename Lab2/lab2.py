...
def make_gaussian_kernel(ksize, sigma):
    ...
    for x in range(ksize):
        for y in range(ksize):
            kernel[x, y] = math.exp(
                ((x - x_mean) ** 2 + (y - y_mean) ** 2) / (-2 * sigma**2)
            )
    # END
    ...
    def img2col(input, h_out, w_out, h_k, w_k, stride):
        ...
        for k in range(h_out * w_out):
            if convwIdx + w_k > w:
                ...
                convhIdx += stride
            out[k] = input[
                convhIdx : convhIdx + h_k, convwIdx : convwIdx + w_k
            ].flatten()
            convwIdx += stride
        return out
...
def estimate_gradients(original_img, display=True):
    ...
    1. Compute dx and dy, responses to the horizontal and vertical Sobel kernel.
    Make use of the cs4243_filter function.
...
def get(d_mag: np.ndarray, x, y):
    h, w = d_mag.shape
    return d_mag[x % h, y % w]


# 3a IMPLEMENT
def non_maximum_suppression_interpol(d_mag: np.ndarray, d_angle, display=True):
    """
    ...
    for x in range(h):
        for y in range(w):
            ...
            tan_theta = abs(tan_angle[x, y])
            now = get(d_mag, x, y)
            if angle < 45:
                side, diag = get(d_mag, x + 1, y), get(d_mag, x + 1, y + 1)
                interpol1 = side + tan_theta * (diag - side)
                side, diag = get(d_mag, x - 1, y), get(d_mag, x - 1, y - 1)
                interpol2 = side + tan_theta * (diag - side)
            elif 45 <= angle < 90:
                side, diag = get(d_mag, x, y + 1), get(d_mag, x + 1, y + 1)
                interpol1 = side + 1 / tan_theta * (diag - side)
                side, diag = get(d_mag, x, y - 1), get(d_mag, x - 1, y - 1)
                interpol2 = side + 1 / tan_theta * (diag - side)
            elif 90 <= angle < 135:
                side, diag = get(d_mag, x, y + 1), get(d_mag, x - 1, y + 1)
                interpol1 = side + 1 / tan_theta * (diag - side)
                side, diag = get(d_mag, x, y - 1), get(d_mag, x + 1, y - 1)
                ...
                side, diag = get(d_mag, x - 1, y), get(d_mag, x - 1, y + 1)
                interpol2 = side + tan_theta * (diag - side)
            if now > interpol1 and now > interpol2:
                out[x, y] = now
    # END
    if display:
...
# 3b IMPLEMENT
def non_maximum_suppression(d_mag: np.ndarray, d_angle, display=True):
    """
...
def find_peak_params(hspace, params_list, window_size=1, threshold=0.5):
    ...
    peak_values = np.array(
        [hspace[tuple(peaks_indices[j])] for j in range(len(peaks_indices))]
    )
    res = []
...
def hough_vote_circles_grad(img: np.ndarray, d_angle: np.ndarray, radius=None):
    ...
    it is default to [3, diagonal of the circle].
    This time, gradient information is used to avoid casting too many unnecessary votes.

    Remember that for a given pixel, you need to cast two votes along the orientation line.
    One in the positive direction, the other in negative direction.
...
def draw_lines(hspace, dists, thetas, hs_maxima, file_path):
    ...
    bounds = [
        np.rad2deg(thetas[0] - angle_step),
